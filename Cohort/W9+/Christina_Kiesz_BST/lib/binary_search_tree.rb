...
class BinarySearchTree
  ...
  def depth(tree_node = @root)
    ...
    # add one for each level, return max from each level going back
    return [1 + self.depth(tree_node.left), 1 + self.depth(tree_node.right)].max

  end

  def is_balanced?(tree_node = @root)
    # two conditions for is balanced
    # a) that the difference is only 1
    # b) that each subtree is balanced left and right
    ...
    return true if (difference <= 1) && left_balanced && right_balanced
    false
  end

  def in_order_traversal(tree_node = @root, arr = [])
    return [] if tree_node.nil?
    # supposed to get the left side, then self, then right side
